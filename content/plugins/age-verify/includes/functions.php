<?php
/*<script>
function check_terms(flag) {

	if ( flag == true ) {
       document.getElementById('av_verify').setAttribute("disabled","disabled");
    } else {
       document.getElementById('av_verify').removeAttribute("disabled");
    }
}
</script>
*/
...
function av_get_verify_form() {
	...
	switch ( $input_type ) {
		...
		case 'dropdowns' :
			...
			$form .='<option style="display:none;" value="" disabled selected>MM</option>';
			foreach ( range( 1, 12 ) as $month ) :
				$form .= '<option value="' . $month . '">' . esc_html( zeroise( $month, 2 ) ) . '</option>';
			endforeach;


			$form .= '</select> &nbsp;&nbsp; <select name="av_verify_d" id="av_verify_d">';
			$form .='<option style="display:none;" value="" disabled selected>DD</option>';
			foreach ( range( 1, 31 ) as $day ) :
				$form .= '<option value="' . $day . '">' . esc_html( zeroise( $day, 2 ) ) . '</option>';
			endforeach;

			$form .= '</select> &nbsp;&nbsp; <select name="av_verify_y" id="av_verify_y">';
			$form .='<option style="display:none;" value="" disabled selected>YY</option>';
			$date = strtotime('-18 years');
			foreach ( range( date( 'Y' , $date), 1910  ) as $year ) :
	...
	//onclick="check_terms(this.checked)"
	$form .= '<div class="terms"><input type="checkbox" id="av_terms" value="1" /> <label for="av_terms"><span></span>By checking this box, you accept <a href="' . get_the_permalink(get_page_by_title("Terms and conditions")->ID).'">the terms of use and privacy policy</a>.</label> </div>';

	$form .= '<div class="submit"><input type="submit" name="av_verify" id="av_verify" value="Enter" /></p>';
